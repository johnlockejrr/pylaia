...
class AdaptiveAvgPool2dTest(unittest.TestCase):
    ...
    def test_identity_tensor(self):
        h, w = 5, 6
        m = torch.nn.AdaptiveAvgPool2d(output_size=(h, w))
    ...
    def test_forward_tensor(self):
        # N x C x H x W
        m = torch.nn.AdaptiveAvgPool2d(output_size=(1, 2))
        y = m(self.x)
...
class AdaptiveMaxPool2dTest(unittest.TestCase):
    ...
    def test_identity_tensor(self):
        h, w = 5, 6
        m = torch.nn.AdaptiveMaxPool2d(output_size=(h, w))
        x = torch.randn(2, 3, h, w, requires_grad=True)
        y = m(x)
        torch.testing.assert_close(y, x)
        (dx,) = torch.autograd.grad(y.sum(), inputs=x)
    ...
    def test_forward_tensor(self):
        m = torch.nn.AdaptiveMaxPool2d(output_size=(1, 2))
        y = m(self.x)
        expected_y = torch.tensor(
            [
                # n = 0
