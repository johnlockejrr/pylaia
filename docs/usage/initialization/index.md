...
The `pylaia-htr-create-model` command can be used to create a PyLaia model.
...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------ |
| `syms`               | Positional argument. Mapping from strings to integers. The CTC symbol must be mapped to integer 0.                                                                                                                              | `bool` | `True`       |
| `config`             | Path to a JSON configuration file                                                                                                                                                                          | `json` | `1000`       |
| `print_config`       | Flag to display current parameters in YAML format                                                                                                                                                          |        |              |
| `fixed_input_height` | Height of the input images. If 0, a variable height model will be used (see `adaptive_pooling`). This will be used to compute the model output height at the end of the convolutional layers.              | `int`  | 0            |
...
| `common.model_filename` | Filename of the model (type: str, default: model) | `str` | `model` |
...
The model can be configured using command-line arguments or a YAML configuration file.
